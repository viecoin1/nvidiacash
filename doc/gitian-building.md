...
*Setup instructions for a gitian build of Nvidiacash using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Nvidiacash
...
- [Building Nvidiacash](#building-nvidiacash)
...
Clone the git repositories for nvidiacash and gitian and then checkout the nvidiacash version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the nvidiacash repository
...
To build Nvidiacash (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the nvidiacash repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/nvidiacash/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/nvidiacash-crypto/nvidiacash
...
For example:
```bash
URL=https://github.com/crowning-/nvidiacash.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit nvidiacash=${COMMIT} --url nvidiacash=${URL} ../nvidiacash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit nvidiacash=${COMMIT} --url nvidiacash=${URL} ../nvidiacash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit nvidiacash=${COMMIT} --url nvidiacash=${URL} ../nvidiacash/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/nvidiacash-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/nvidiacash-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/nvidiacash-build.assert
```
...
[nvidiacash/gitian.sigs](https://github.com/nvidiacash-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
